function  [pickA1,pickBB,flags] = sxk_RefineCorrection_GR_NPHI_tail(a1,b1,pickA0,s1_scope,p1_refine,tensile_pool,pad_numval)
...
jud_loga = (a1log~=3.*pad_numval & a1log~=pad_numval) & (a2log~=2.*pad_numval & a2log~=pad_numval);
jud_logb = (b1log~=3.*pad_numval & b1log~=pad_numval) & (b2log~=2.*pad_numval & b2log~=pad_numval);
...
if isempty(pickB1set)==1
    pickB1set = sub_deterimineBpicks(pickA1,p1_refine*2,idxvec_Beff,b1log_eff+b2log_eff,space_L1);
end 
...
obj2d   = sub_ObjFun_FrontPart_retune_GR_NPHI(loga1_tar,logb1_tar,pickA1_rel,pickB1s_rel,tensile_pool,min_coverlen);
...
% space_L2 = 1;
% pickB2set = sub_deterimineBpicks(pickB1,space_L1*4,idxvec_Beff,b1log_eff+b2log_eff,space_L2);
% 
...
% 
% obj2d   = sub_ObjFun_FrontPart_retune_GR_RD(loga2_tar,logb2_tar,pickA1_rel,pickB2s_rel,tensile_pool,min_coverlen);
% 
% [maxval,maxidx1d] = max(obj2d(:));
% [maxridx,maxcidx] = ind2sub(size(obj2d),maxidx1d);
% obj2d_mf          = medfilt2(obj2d,[11 11]);
