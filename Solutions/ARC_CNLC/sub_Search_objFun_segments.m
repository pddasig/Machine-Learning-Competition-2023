function [obj_simi,obj_area] = sub_Search_objFun_segments(loga0,logb0,nrad_corr)
...
obj_loga = zeros(1,ndep_log);
obj_logb = zeros(1,ndep_log);
...
for in = 1:1:num_CO    
    ...
    aamat(judmat) = loga_ii(idxmat(judmat))+0.001; %%%%%% !!!!!!
    bbmat = zeros(nrad_corr.*2+1,len_ii);
    bbmat(judmat) = logb_ii(idxmat(judmat))+0.001; %%%%%% !!!!!!   
...
%     area_avg = 0.5.*(loga_ii+logb_ii).';  %%% method 01
    area_avg = 0.5.*(min(abs(aamat),[],1) + min(abs(bbmat),[],1)); %%% method 02
    area_vec = abs(area_vec-area_avg);            

%     log_base  = 0.5.*(loga_ii+logb_ii).';  %%% method 01
%     area_loga = abs(median(aamat) - log_base);
%     area_logb = abs(median(bbmat) - log_base);
%     area_vec  =  abs(area_loga-area_logb);  

    simi_vec  = mean(aamat.*bbmat)./(mean(aamat.^2+bbmat.^2));    
    obj_simi(idx01:idx02) = (simi_vec);
    obj_area(idx01:idx02) = area_vec;
%     obj_loga(idx01:idx02) = area_loga;
%     obj_logb(idx01:idx02) = area_logb;
