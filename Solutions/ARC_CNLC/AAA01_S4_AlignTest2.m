...
test_csv_file = 'test_well_02.csv';
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%% step 02 :: processing the training dataset
SEridx = cumsum(SE_ridx,1);
SEridx = reshape(SEridx(:),[2 num_trainfile]);

trainfile_idx = 1;  %%% here I use the 7th index
sidx = SEridx(1,trainfile_idx);
eidx = SEridx(2,trainfile_idx);
dat_train = trainMat(sidx:eidx,:);
Train.depth = dat_train(:,1);
Train.gr    = dat_train(:,2);
Train.rhob  = dat_train(:,3);
Train.nphi  = dat_train(:,4);
Train.rd    = dat_train(:,5);
Train.gr_trend   = medfilt2(Train.gr,[mflenSEP 1]);
Train.rhob_trend = medfilt2(Train.rhob,[mflenSEP 1]);
Train.nphi_trend = medfilt2(Train.nphi,[mflenSEP 1]);
Train.rd_trend   = medfilt2(Train.rd,[mflenSEP 1]);
Train.gr_detail       = Train.gr-Train.gr_trend;
Train.rd_detail       = Train.rd-Train.rd_trend;
Train.nphi_detail     = Train.nphi-Train.nphi_trend;
Train.rhob_detail     = Train.rhob-Train.rhob_trend;
