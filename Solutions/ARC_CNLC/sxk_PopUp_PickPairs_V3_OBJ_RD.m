function  [pickpairs_out,flags_out,a1r,b1r] = sxk_PopUp_PickPairs_V3_OBJ_RD(a1,b1,s0_scope,s1_scope,pad_numval)
...
[pickA1,pickB1,flag1] = sxk_RefineCorrection_GR_RD_head(a1,b1,a1idx_head,s1_scope,p1_refine,tensile_pool,pad_numval);
[pickA2,pickB2,flag2] = sxk_RefineCorrection_GR_RD_tail(a1,b1,a1idx_tail,s1_scope,p1_refine,tensile_pool,pad_numval);
...
for ip = 1:1:length(Apick_opt)
    pickAi = Apick_opt(ip);
    [pickAii,pickBii,flagval] = sxk_RefineCorrection_GR_RD(a1,b1,pickAi,s1_scope,p1_refine,tensile_pool,pad_numval);
    
    pickpairs = [pickpairs; [pickAii pickBii(end)]];
    flags     = [flags;flagval];    
end
jud_standout = (flags>=0.32 & flags<10) & isinf(abs(flags))==0  ;
pickpairs_out    = pickpairs(jud_standout,:);
flags_out        = flags(jud_standout,:);  

if abs(pickB1(end)-pickA1) < 4
    pickB1(end)= pickA1;
end
if abs(pickB2(end)-pickA2) < 4
    pickB2(end)= pickA2;
end
