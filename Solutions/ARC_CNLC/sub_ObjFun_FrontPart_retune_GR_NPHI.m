function  obj2d_com = sub_ObjFun_FrontPart_retune_GR_NPHI(loga00,logb00,pickA0,pickB0s,tensile_pool,min_coverlen)
...
for itop = 1:1:ntry     
   ...
   parfor itf = 1:1:nten                   
        ...
        nrad_corr = 8;
        [obj_simi,obj_area] = sub_objFun_GR_NPHI_detail(loga2,logb2,nrad_corr);        
        objvec_simi   = sqrt(abs(obj_simi));
        objval_si     = norm(objvec_simi,2);               
        objvec_area   = obj_area; 
        objval_ei     = sum((objvec_area));      
...
function [obj_simi,obj_area,loc_refCO,loc_rawCO] = sub_objFun_GR_NPHI_detail(loga0,logb0,nrad_corr)
...
for in = 1:1:num_CO    
    ...
    simi_vec  = mean(aamat.*bbmat)./(mean(aamat.^2+bbmat.^2)+1e-8);    
    obj_simi(idx01:idx02) = abs(simi_vec);
