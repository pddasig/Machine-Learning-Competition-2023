function [r0,t0,rtidx,d,w1] = sxk_DTW_newobj_Shift_iter_dist(r,t,rpidx,tpidx,scope_nrad,max_shift,pad_numval,picks_now)
...
% %         obj_dist(im,in) = sqrt(mean(avec0./max(abs(avec0))-bvec0./max(abs(bvec0))).^2);
% %     end
% % end
% % obj_dist = obj_dist./max(obj_dist(:));
% % obj_dist = obj_dist - median(obj_dist,1);
% % obj_dist = obj_dist - median(obj_dist,2);
% % figure(111)
% % clf;
% % subplot(221)
...
% % subplot(223)
% % imagesc(obj_dist);colorbar;
% % subplot(224)
% % imagesc(obj_corr+obj_dist+obj_area);colorbar;


d =  (obj_corr);
% d = (obj_dist).*(obj_corr+obj_area);
